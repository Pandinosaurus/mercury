...
The **ChatInput** widget provides a multiline text input field with a send button, designed for
...
Use `Shift+Enter` to insert a new line. If `send_on_enter=False`, Enter inserts
new lines instead of submitting the message.

While the app is generating a response, the send button automatically changes to
`Stop`. Clicking it interrupts the running kernel execution and preserves the text
currently typed in the input.

<Aside>
This reset behavior makes it easy to treat `prompt.value` as a “one-shot” input:
